...
flags.DEFINE_string('weights', os.getenv('HOME') + '/wego_ws/src/scout_mini_ros/scout_bringup/checkpoints/yolov4-tiny-416',
...
if use_webcam:
    flags.DEFINE_string('video', '4', 'path to input video')
...
def main(_argv):
    ...
    model_filename = os.getenv('HOME') + '/wego_ws/src/scout_mini_ros/scout_bringup/model_data/mars-small128.pb'
    ...
    # begin video capture
        try:
            ...
            # vid.set(cv2.CAP_PROP_POS_MSEC, 100)
        except:
    ...
    while not rospy.is_shutdown():
        ...
        # 좌/우 회전 한곗값 설정
        left_limit = frame.shape[1]//2 - 200
        right_limit = frame.shape[1]//2 + 200
        ...
        if tracker.lost: # 추적할 객체가 없다면 정지
            ...
            for track in tracker.tracks:
                ...
                # depth camera 없으면 detection한 bbox의 width를 통해 person_distance 구함
                # width가 넓으면 가까이, 좁으면 멀리 있다고 가정 적당한 값 w에 곱해주기
                if use_webcam:
                    person_distance = w * 5
